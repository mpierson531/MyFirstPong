...
import com.mygdx.Pong.ComPlayer;
...
public class OnePlayerPong implements Screen {
	...
	public OnePlayerPong(Game game, SpriteBatch spriteBatch, Artist2D artist2D) {
		this.game = game;
		...
		playerOne = new Player(40, 50f, 30f, 100f);
		comPlayer = new ComPlayer();
		ball = new Circle(Gdx.graphics.getWidth()/2f, Gdx.graphics.getHeight()/2f, 10,
	...
	public void render(float delta) {
		...
		scoreUI.draw();
		spriteBatch.begin();
		scoreUI.drawScores(playerOne.getScore(), comPlayer.getScore(), spriteBatch);
		...
		spriteBatch.draw(paddleSprite, comPlayer.getRectangle().getX(), comPlayer.getRectangle().getY());
	...
	private void resetBall() {
		...
		float randomX = MathUtils.randomSign();
		ball.setVelocityX((randomX == -1) ? -Constants.MAX_BALL_SPEED : Constants.MAX_BALL_SPEED);
